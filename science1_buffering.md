...
> The image shows 2 possible trajectories at the very start of the eyeball room. The red trajectory is theoretically faster and is more viable to make the cycle later in the room because of the extra +40 speed from a bhop. However, it is difficult to modulate the jump height properly and a small misstep can lead to catastrophic failure. The green trajectory, which is what I aim for and is illustrated in the animation, uses a single big jump and removes the variable of modulating the jump height. On average, the green trajectory is liable to have less speed than the red trajectory, which may make the cycle more difficult. In order to make the green trajectory more successful at making the cycle, I pay attention to the details of the room entry to make it as fast and consistent as possible to compensate for not getting the +40 compared to the red trajectory.
